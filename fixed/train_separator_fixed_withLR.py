...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        if writer is not None and batch_idx % hp.log_every_n_steps == 0:
            global_step = epoch * num_batches + batch_idx
            writer.add_scalar('Train/batch_loss', loss.item(), global_step)
            writer.add_scalar('Train/batch_pit_loss', logs["loss/pit_latent"].item(), global_step)
            writer.add_scalar('Train/gradient_norm', grad_norm, global_step)
            writer.add_scalar('Train/learning_rate', current_lr, global_step)
            
            # Log all components from loss dict
...
@torch.no_grad()
def evaluate(model, val_loader, hp: FixedHParams, writer: Optional[SummaryWriter] = None, epoch: int = 0):
    ...
    for batch in tqdm(val_loader, desc="Validation"):
        ...
        total_pit_loss += logs["loss/pit_latent"].item()
        num_batches += 1
    
    avg_loss = total_loss / max(1, num_batches)
    avg_pit_loss = total_pit_loss / max(1, num_batches)
    
    # TensorBoard logging for validation
    if writer is not None:
        writer.add_scalar('Val/loss', avg_loss, epoch)
        writer.add_scalar('Val/pit_loss', avg_pit_loss, epoch)
...
def main():
    ...
    if hp.resume_from_checkpoint:
        print("\n" + "=" * 60)
        print(f"Resuming from checkpoint: {hp.resume_from_checkpoint}")
        print("=" * 60)
        
        if not Path(hp.resume_from_checkpoint).exists():
            print(f"❌ Checkpoint not found: {hp.resume_from_checkpoint}")
