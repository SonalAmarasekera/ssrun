...
# -----------------------------------------------------------------------------
class SeparationHeadGated(nn.Module):
    """Drop-in replacement with gating."""
    def __init__(self, dim: int, hidden: int, num_sources: int, mode: str = "residual"):
        ...
        # NEW: Split into transform + gate branches
    ...
    def forward(self, x: torch.Tensor, x_ref: torch.Tensor) -> torch.Tensor:
        h = self.pre(x)  # [B, T, hidden]
        
        # Gated output
        transform = self.output_transform(h)
        gate = self.output_gate(h)
        y = (transform * gate).view(x.size(0), x.size(1), self.num_sources, -1)
...
class RWKVv7Separator(nn.Module):
    def __init__(self, cfg: SeparatorV7Config):
        ...
        self.head = SeparationHeadGated(C, cfg.head_hidden, cfg.num_sources, mode=cfg.head_mode)
