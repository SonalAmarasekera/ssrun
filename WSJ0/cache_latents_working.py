...
def _write_pt(save_path: pathlib.Path, z: torch.Tensor):
    # inside _write_pt in cache_latents_working.py, before torch.save(...)
    z = z.cpu()
    if z.ndim == 3 and z.shape[0] == 1:       # [1,C,T] -> [C,T]
        z = z.squeeze(0)
    if z.shape[0] % 64 == 0:                  # [C,T] -> [T,C]
        z = z.transpose(0, 1).contiguous()
    #meta = {"z": z, "sr": expect_sr, "fps": 50}  # set your known latent fps
    sr_value = getattr(_CODEC, "sample_rate", 16000)
    meta = {"z": z.cpu(), "sr": sr_value, "fps": 50}
