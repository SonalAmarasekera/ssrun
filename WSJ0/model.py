...
if 'x070' in os.environ["RWKV_MY_TESTING"]:
    ...
    class WindBackstepping(torch.autograd.Function):
        ...
        def forward(ctx, w,q,k,v,z,b):
            B,T,H,C = w.shape 
            assert T%CHUNK_LEN == 0, "Error 1" # if T%CHUNK_LEN != 0: pad your input to T%CHUNK_LEN == 0, or change CHUNK_LEN (will be slower)
            assert all(i.dtype==torch.bfloat16 for i in [w,q,k,v,z,b]), "Error 2"
            assert all(i.is_contiguous() for i in [w,q,k,v,z,b]), "Error 3"
...
class RWKV_Tmix_x070(MyModule):
    ...
    def forward(self, x, v_first):
        ...
        r, w, k, v, kk, a = [t.to(torch.bfloat16).contiguous() for t in (r, w, k, v, kk, a)]
