...
class HParams:
    ...
    val_root:   str = "/content/latents/val"
    batch_size: int = 8
    num_workers: int = 4
...
def crop_batch_to_seconds(batch, seg_seconds: float):
    """Crop each sample to ~seg_seconds using fps; preserves [B,T,C] and mask."""
    z_mix, z_s1, z_s2, mask, fps = batch["z_mix"], batch.get("z_s1"), batch.get("z_s2"), batch["mask"], batch["fps"]
    B, T, C = z_mix.shape
    # use per-batch fps mode (assume uniform in batch; assert)
    assert int(fps.min()) == int(fps.max()), "Batch contains mixed fps; bucket by fps in DataLoader if needed."
    frames = int(seg_seconds * int(fps[0]))
    if T <= frames:  # no crop
        return batch
...
def main():
    ...
    # Data
    train_ds = RWKVLatentDataset(hp.train_root, require_targets=True)
    val_ds   = RWKVLatentDataset(hp.val_root,   require_targets=True)
    train_loader = DataLoader(train_ds, batch_size=hp.batch_size, shuffle=True,
                              num_workers=hp.num_workers, pin_memory=hp.pin_memory,
                              collate_fn=collate_rwkv_latents, drop_last=True)
    val_loader   = DataLoader(val_ds, batch_size=hp.batch_size, shuffle=False,
                              num_workers=hp.num_workers, pin_memory=hp.pin_memory,
                              collate_fn=collate_rwkv_latents, drop_last=False)
    ...
    for epoch in range(hp.epochs):
        ...
        for batch in train_loader:
            ...
            batch = crop_batch_to_seconds(batch, hp.seg_seconds)
