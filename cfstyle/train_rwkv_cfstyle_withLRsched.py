...
from rwkv_separator_Gemini import build_rwkv7_separator  # your RWKV v7 separator
...
def train_one_epoch(
    ...
    device: torch.device,
    grad_clip: float = 5.0,
    writer: SummaryWriter = None,   # NEW: for intra-epoch logging
) -> float:
    model.train()
...
def validate(
    ...
    for mix, sources in pbar:
        ...
        src_q, _, _, _, _ = dac.get_quantized_features(src_enc)
        codec_src_flat = dac.get_decoded_signal(src_q, src_orig_len)
        codec_sources = codec_src_flat.view(B, S, T)
...
def main():
    ...
    ap.add_argument("--log_dir", type=str, default="/root/checkpoints_rwkv_dac/tb",
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        train_loss = train_one_epoch(
            ...
            device,
            grad_clip=args.grad_clip,
            writer=writer,         # NEW: enable step-wise logging
        )
